    ...
    public class SQLHelper
    {
        ...
        public static DataTable GetPageListByProc(string proName, int pageIndex, int pageSize, bool isDel, out int pageCount, out int rowCount)
        {
            ...
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                da.SelectCommand.Parameters.AddWithValue("@isDel", isDel);//页容量

                //在存储过程中 输出参数
                da.SelectCommand.Parameters.Add(new SqlParameter("@pageCount", SqlDbType.Int));//总页数
                da.SelectCommand.Parameters.Add(new SqlParameter("@rowCount", SqlDbType.Int));//总行数
                //将后面两个参数 设置为 输出类型
                ...
                rowCount = Convert.ToInt32(da.SelectCommand.Parameters[4].Value);
            }
            //返回数据表
            return dt;
        }
